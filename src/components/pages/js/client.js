...
const ClientPage = (props) => {
  ...
  const idSucursal = props.user.userLogged.userProperties.id_sucursal;
  useEffect(() => {
    getClientByIdSucursal(idSucursal).then(({ data }) =>
      props.AddClient(data.body)
    );
    getCities().then(({ data }) => props.AddCities(data.body));
  }, []);
  return (
    <BaseTemplate history={props.history}>
      <div className='product-page'>
        <div className='product-page__title'>
          <h4 className='product-page__title--normal'>CLIENT</h4>
        ...
        <div
          ...
          onClick={() => {
            SetModal(!modal);
            SetformNewConfig(formClient("", "", "", "", "", "", props.cities));
          }}
        >
        ...
        <div className='product-page__table'>
          {props.listClient ? (
            <TableOrganism
              data={props.listClient}
              label={tableClientConfiguration}
            />
          ) : (
            "Loading"
          )}
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators({ AddClient, AddCities }, dispatch);
